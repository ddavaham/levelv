...
class SkillQueue extends Command
{
    public $dataCont;
     ...
     * @var string
     */
    protected $signature = 'update:skillqueue';
    ...
    {
        $now = now(); $x=0;
        $members = Member::where('disabled', 0)->whereRaw("JSON_CONTAINS(scopes, '[\"esi-skills.read_skillqueue.v1\"]')")->chunk(50, function ($chunk) use (&$now,&$x) {
            $chunk->each(function ($member) use (&$now,&$x) {
                $job = new GetMemberSkillQueue($member->id);
                $job->delay($now);
                Bus::dispatch($job);
